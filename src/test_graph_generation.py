...
def test_connected_directed_graph():
    ...
    for i in range(10):
        graph = generate_connected_graph(100, 100, True)
        unconnected_induced_graph = graph.induced_graph(graph.vertices, False) # erreur too many positional arguments du linter, mais le code passe quand même
        assert unconnected_induced_graph.is_connected() == True, "One of the graphs isn't connected : \n" + str(graph)
...
def test_random_directed_graph():
    ...
    assert edges == 9900, "One of the graphs doesn't have the right number of edges : " + str(edges) +  "for : \n" + str(graph)
...
def test_random_community_graph():
    for i in range(10):
        ...
        assert len(graph1.vertices) == sum([5*i,3*i,4*i]), "Graph doesn't have the right number of edges\n" + str(graph1)
    ...
    graph = generate_random_community_graph(nodes_per_community, p_intra, p_inter)
    ...
    solid_community = 0 # number of community whose number of edges is within intra_trust interval
    successful_community = 0 # number of community whose number of edges is within inter_trust interval


    for community in communities:
        
        intra_trust_min = len(community)*(len(community)-1)/2 * p_intra - 1.96 * sqrt(len(community)*(len(community)-1)/2 * p_intra * (1 - p_intra))
        intra_trust_sup = len(community)*(len(community)-1)/2 * p_intra + 1.96 * sqrt(len(community)*(len(community)-1)/2 * p_intra * (1 - p_intra))

        inter_trust_min = len(community)*(len(graph.vertices) - len(community)) * p_inter - 1.96 * sqrt(len(community)*(len(graph.vertices) - len(community)) * p_inter * (1 - p_inter))
        inter_trust_sup = len(community)*(len(graph.vertices) - len(community)) * p_inter + 1.96 * sqrt(len(community)*(len(graph.vertices) - len(community)) * p_inter * (1 - p_inter))
    ...
    print("Test 3.1 : " + str(solid_community) + " of 100 communities are solid")
    print("Test 3.2 : " + str(successful_community) + " of 100 communities are successful")
